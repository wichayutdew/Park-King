...
//*******************************************************Reserve's Adder***********************************************
function Reserve(qrcodeIn,qrcodeOut,timeIn,timeOut,reserveid,username,floor,slot,buildingname){
  var request = new Request("INSERT INTO dbo.Reserve (QRCodeIn,QRCodeOut,Time_In,Time_Out,ReserveID,Username,Floor,Slot,BuildingName) values (@QRCodeIn,@QRCodeOut,@Time_In,@Time_Out,@ReserveID,@Username,@Floor,@Slot,@BuildingName)",
  //CustomerPicture,profilePic
      function (err, rowCount, rows){
          if(err){
              done(err);
